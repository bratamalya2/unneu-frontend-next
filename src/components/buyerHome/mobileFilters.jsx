...
export default function MobileFilters({ appliedFilters, addFilter, removeFilter, removeAllFilters, showMobileFilters, handleCloseMobileFilters }) {
    ...
    return <section className={`${showMobileFilters ? "w-full h-fit mt-[30px] pb-[50px]" : "w-0 h-0"} transition-[width] duration-500 ease-in-out relative border-y-2 border-y-black rounded-t-[10px] sticky bg-white top-[20vh] z-50 overflow-y-auto`}>
        ...
        <section className="mt-[35px] w-full">
            ...
            <div className="py-[12px] px-[5%] flex flex-row flex-nowrap items-center justify-between font-medium">
                By Occasion
                <Image src={showOccasion ? UpArrow : DownArrow} alt="down" className="w-[11px] h-[5px]" onClick={() => {
                    setShowOccasion(x => !x);
                }} />
            </div>
            <div className={`w-full h-[1px] ${showOccasion ? "bg-black" : "bg-[#DDD]"}`}></div>
            {
                showOccasion && (
                    <>
                        <div className="py-[12px] px-[5%] flex flex-row flex-nowrap items-center justify-between text-sm">
