...
export default function SellerPickupForm({ sellerDetails }) {
    ...
    const handleFormSubmit = async () => {
        try {
            if (fullName.length === 0)
                ...
                const x = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/seller/editProfile/2`, {
                    method: "POST",
                    body: formdata
                });
                const y = await x.json();
                console.log(y);
                if (y.success)
                    router.push("/seller/editProfile/3");
                else
                    enqueueSnackbar(y.err, {
                        variant: "error"
                    });
    ...
    useEffect(() => {
        if (sellerDetails) {
            console.log(sellerDetails.state);
            setFullName(sellerDetails.fullName);
            setStoreName(sellerDetails.storeName);
            setGender(sellerDetails.gender);
            setAddress(sellerDetails.address);
            setContactPhoneNumber(sellerDetails.contactPhoneNumber);
            setPincode(sellerDetails.pincode);
    ...
    return <section className="mt-[110px] lg:mt-[250px] pl-[10%] lg:pl-[25%] pr-[10%] lg:pr-[5%] mb-[50px] lg:mb-[100px] flex flex-col flex-nowrap">
        ...
        <div className="mt-[21px] lg:mt-[42px] flex flex-row flex-nowrap items-center text-xl gap-x-4 lg:gap-x-7">
            <div className="flex flex-row flex-nowrap items-center">
                <input type="radio" id="male" name="gender" value="Male" checked={gender === "Male"} className="mr-[13px] h-[16px] w-[16px] local-custom-radio" onClick={() => setGender("Male")} />
                <label htmlFor="male" className="text-[15px] lg:text-base">Male</label>
            </div>
            <div className="flex flex-row flex-nowrap items-center">
                <input type="radio" id="female" name="gender" value="Female" checked={gender === "Female"} className="mr-[13px] h-[16px] w-[16px] local-custom-radio" onClick={() => setGender("Female")} />
                <label htmlFor="female" className="text-[15px] lg:text-base">Female</label>
            </div>
            <div className="flex flex-row flex-nowrap items-center">
                <input type="radio" id="other" name="gender" value="Other" checked={gender === "Other"} className="mr-[13px] h-[16px] w-[16px] local-custom-radio" onClick={() => setGender("Other")} />
        ...
        <div className="w-full lg:w-[60%] border border-[#CACACA] rounded-[24px] p-[16px] mt-[24px] lg:mt-[55px]">
            ...
            {showAddressForm && <div className="lg:mx-[25px]">
                ...
                <select name="state" id="state" className="mt-[12px] rounded-[12px] border-[0.6px] border-[#00000080] bg-[#F9F9F9] h-[52px] w-full px-3 custom-select" onChange={e => {
                    ...
                    {
                        States.map((s, i) => (
                            <option key={i} value={s} selected={s === state}>{s}</option>
                        ))
                    }
                </select>
